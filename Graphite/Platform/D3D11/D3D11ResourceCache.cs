...
internal unsafe class D3D11ResourceCache : IDisposable
{
    ...
    private ComPtr<ID3D11BlendState> CreateNewBlendState(ref BlendStateDescription description)
    {
        ...
        BlendDesc d3dBlendStateDesc = new();
    ...
    {
        RasterizerDesc rssDesc = new()
        {
            CullMode = D3D11Formats.VdToD3D11CullMode(key.GraphiteDescription.CullMode),
            FillMode = FillMode.Solid,
            DepthClipEnable = key.GraphiteDescription.DepthClipEnabled,
            ScissorEnable = key.GraphiteDescription.ScissorTestEnabled,
            FrontCounterClockwise = key.GraphiteDescription.FrontFace == FrontFace.CounterClockwise,
    ...
    private struct D3D11RasterizerStateCacheKey : IEquatable<D3D11RasterizerStateCacheKey>
    {
        ...
        {
            return GraphiteDescription.Equals(other.GraphiteDescription)
                && Multisampled.Equals(other.Multisampled);
        }

        public override int GetHashCode()
        {
            return HashCode.Combine(GraphiteDescription.GetHashCode(), Multisampled.GetHashCode());
        }
    }
}
