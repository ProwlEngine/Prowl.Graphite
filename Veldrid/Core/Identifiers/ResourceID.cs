...
/// <summary>
/// Interned identifier for a shader resource binding name (uniform, texture, sampler, etc.).
/// Cheap value-type wrapper around a process-wide integer minted by an internal interner.
/// </summary>
[DebuggerDisplay("{ToString(),nq}")]
public readonly struct ResourceID : IEquatable<ResourceID>, IFormattable
{
    internal readonly int Value;

    internal ResourceID(int value) { Value = value; }
    ...
    private static readonly Interner<string, ResourceID> s_interner =
        new(static _ => new ResourceID(Interlocked.Increment(ref _counter)));

    /// <summary>
    /// Returns the ID for <paramref name="name"/>, minting one if this is the first time
    /// this string has been seen.
    /// </summary>
    public static ResourceID Intern(string name) => s_interner.Intern(name);
    ...
    /// </summary>
    public static string? ToString(ResourceID id)
        => s_interner.TryGetKey(id, out string? key) ? key : null;

    /// <summary>
    /// Implicit string-to-ID conversion. Equivalent to <see cref="Intern(string)"/>.
    /// </summary>
    public static implicit operator ResourceID(string name) => Intern(name);

    public bool Equals(ResourceID other) => Value == other.Value;
    public override bool Equals(object? obj) => obj is ResourceID o && Equals(o);
    public override int GetHashCode() => Value;
    public static bool operator ==(ResourceID a, ResourceID b) => a.Value == b.Value;
    public static bool operator !=(ResourceID a, ResourceID b) => a.Value != b.Value;

    /// <summary>
    /// Hot-path safe. Does not touch the interner. Use the static <see cref="ToString(ResourceID)"/>
