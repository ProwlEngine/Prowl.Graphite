...
public class BackendInfoOpenGL
{
    ...
    /// <summary>
    /// Gets the name of the OpenGL texture object wrapped by the given Prowl.Graphite Texture.
    /// </summary>
    /// <returns>The Prowl.Graphite Texture's underlying OpenGL texture name.</returns>
    public uint GetTextureName(Texture texture) => Util.AssertSubtype<Texture, OpenGLTexture>(texture).Texture;

    /// <summary>
    /// Sets the texture target of the OpenGL texture object wrapped by the given Prowl.Graphite Texture to to a custom value.
    /// This could be used to set platform specific texture target values like Prowl.Graphite.OpenGLBinding.TextureTarget.TextureExternalOes.
    /// </summary>
